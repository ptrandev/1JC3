...
isBST (Leaf v) minVal maxVal = checkMin minVal v && checkMax maxVal v
isBST (Node v left right) minVal maxVal = 
  checkMin minVal v && checkMax maxVal v && isBST left minVal (Just v) && isBST right (Just v) maxVal

checkMin :: Ord a => Maybe a -> a -> Bool
checkMin Nothing _ = True
checkMin (Just minVal) v = v >= minVal

checkMax :: Ord a => Maybe a -> a -> Bool
checkMax Nothing _ = True
checkMax (Just maxVal) v = v < maxVal
