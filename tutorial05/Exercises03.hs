...
-----------------------------------------------------------------------------------------------------------
-- Helper function to check if all elements in a tree are less than a given value
allLessThan :: Ord a => a -> Tree a -> Bool
allLessThan x (Leaf a) = a < x
allLessThan x (Node a left right) = a < x && allLessThan x left && allLessThan x right

-- Helper function to check if all elements in a tree are greater than or equal to a given value
allGreaterOrEqual :: Ord a => a -> Tree a -> Bool
allGreaterOrEqual x (Leaf a) = a >= x
allGreaterOrEqual x (Node a left right) = a >= x && allGreaterOrEqual x left && allGreaterOrEqual x right

-- Main function to check if a tree is a search tree
isSearchTree :: Ord a => Tree a -> Bool
isSearchTree (Leaf _) = True
isSearchTree (Node v left right) = 
    allLessThan v right && allGreaterOrEqual v left && isSearchTree left && isSearchTree right
...
-----------------------------------------------------------------------------------------------------------
factors :: Int -> [Int]
factors n = filter (\x -> n `mod` x == 0) [2..n-1]
